...
from \lnox in the mid-troposphere, altitude at which positive net \chem{O_3} attended within the anticyclone is gradually
...
In the upper troposphere, where \chem{NO_x} shows substantial superlinear increases due to {\lnox} between $0.1\times$
and $1\times$ lightning. Consequently, such high presence of \chem{NO_x} induces reversal in the chemical production
of \chem{O_3}. \chem{NO_x} titration has been suggested to be possible during intense thunderstorms \citep[e.g.][]{Cummings:2013vn}.
However, persistent occurrence indicated in Figure~\ref{fig:2006/ltngsens_vertical} is likely due to overestimation of
...
VMR is minimal, and thus only daytime VMR distributions are shown. On top of the mean values, the min-max ranges as well as the
5--95 percentiles are shown. Due to the interconversion between \chem{O_3} and \chem{NO_2}, we examine odd oxygen
\chem{O_x\equiv O_3+NO_2} in the following analysis.

While the differences between ozone VMRs between scenarios are monotonic and within the confines of 25\% between $0\times$
...
is slightly increased to 0.403\,\unit{ppbv/hr}, unlike what is shown in the all-day accumulated profiles. On the other hand, ozone
...
to 0.033\,\unit{ppbv/hr} and the minimum (largest potential loss) is s decreased from $-0.093$ to $-0.853\,\unit{ppbv/hr}$, thus 
...
reserved and the net loss at 5-percentile is now $-255\,\unit{pptv/hr}$. These data points correspond to high-{\lnox} air
...
surface VOC and \chem{NO_x} sources are decreased. These changes are expected to cause substantial changes in the ozone
chemical tendency and VMR on the surface, which is then propagated to the upper troposphere through convection. Due to
the highly non-uniform emission density, transport-dispersion patterns are expected from source regions such as cities

\subsection{Biogenic emission}\label{ssec:2006/sens/bio}

Despite lower emission density (per area) at source, widespread coverage of biogenic emission contributes significantly to
tropospheric ozone production (see Sect.~\ref{ssec:intro/ozone/voc}). Similar to Section~\ref{ssec:2006/sens/anthrop}, we
quantify the contribution of biogenic emission to the ozone enhancement by conducting a simulation without biogenic
emission. In doing so, species such as isoprene and formaldehyde are decreased to
