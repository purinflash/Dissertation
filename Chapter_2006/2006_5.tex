...
tracers are dynamically-driven, i.e. independent of chemistry, it can be confidently asserted that the impact of air from the boundary
condition on the ozone enhancement is minimal.

The lightning \chem{NO_x} tracer (LT) also shows substantial localized enhancement within the anticyclone (Fig.~\ref{fig:2006/tracers}{\bf g}).
...
Similarly, the stratospheric (ST) tracer shows very minimal impact from the NAM circulation except for the influx of air with high ozone associated with mesoscale
...
Same as Figure~\ref{fig:2006/tracers} except for tracer equivalent age. High variance data points (tracer values$<10^{-4}$) have been masked out.}{0.9}
...
Model simulated mean {\bf(a)} \chem{NO_x}, {\bf(b)} \chem{HO_x}, {\bf(c)} \chem{CO}, and {\bf(d)} \chem{O_3} for $0\times$ (left column), $0.1\times$ (center column), and $1\times$ (right column)
...
August mean anticyclone VMR for \chem{CO} is $92.3\pm2.8\,\unit{ppbv}$. This is reduced to $89.8\pm2.4\,\unit{ppbv}$ with $0.1\times$ lightning
...
\chem{CO}, and \chem{NO_x} from the base case and sensitivity simulations averaged within the anticyclone.
With the exception of \chem{O_3} and \chem{NO_x} in the $1\times$ lightning setting, the
shapes of the mixing ratio vertical profiles are consistent across simulations. The greatest effect of the lightning-\chem{NO_x}
...
\chem{NO_x} is enhanced most prominently at the 169\,\unit{hPa}level  from 84\,\unit{pptv} to 3.09\,\unit{ppbv}. A more detailed
...
In the upper troposphere, \chem{NO_x} increases superlinearly due to {\lnox} emissions between $0.1\times$
and $1\times$ lightning flash rate simulations, suggesting feedback. Consequently, the presence of high
\chem{NO_x} induces a decrease in the chemical production
...
\chem{NO_x} VMR, a substantial amount of  \chem{HO_x} radicals are consumed by the reaction \chem{OH+NO_2\rightarrow HNO_3},
...
mixing ratio for \chem{HNO_3} between 150-300\,\unit{hPa} during August within the anticyclone. Nighttime (3--9\,\unit{UTC}) tendencies are
represented by dashed lines while daytime (15--21\,\unit{UTC}) tendencies are represented by solid lines.}
...
%%%% Rewrite this paragraph
Changes in the ozone chemical tendencies between lightning emission scenarios are substantial. Without lightning, the
...
daytime \chem{NO_x}-titration is occurring albeit not as severe as indicated in Figure~\ref{fig:2006/ltngsens_vertical}.

By comparing the changes in the distributions of the chemical tendencies for \chem{O_3} and \chem{O_x}, it is apparent that
the bulk of ozone losses at $1\times$ is due to conversion to \chem{NO_2}. This argument is supported by the increase
in \chem{NO_x} as well as \chem{HNO_3}, which is increased from 6.6\,\unit{ppbv} at $0.1\times$ lightning to 39.6\,\unit{ppbv}
...
with a mean of $-46\,\unit{pptv/hr}$. This indicates that sufficient \chem{CO} precursors are present in the upper
troposphere to counter the loss due to reaction with \chem{OH} radicals. The addition of $0.1\times$ {\lnox}
moves this range to $-281$--$3\,\unit{pptv/hr}$, thus doubling the net loss on the low end and essentially erasing any
production. This is likely caused by losses of VOCs due to oxidation at lower levels, thus leaving less precursors available for in situ \chem{CO}
...
To understand the source of the ozone chemical production reversal seen in the model results for high \chem{NO_x}
...
reactions $L_R$ (e.g. \chem{HO_2+HO_2}) and radical-\chem{NO_x} reactions $L_N$ (e.g. \chem{NO_2+OH}).
That is, $Q=L_R+L_N$. The response of ozone production to the available \chem{NO} can then be quantified
...
\end{equation}
where $L_N/Q$ is the fraction of free radicals removed by \chem{NO_x}. In the upper troposphere, $L_N$ and $L_R$ can be
approximated by the reaction rate of \chem{NO_2+OH} and twice the rate of \chem{HO_2+HO_2}, respectively. We conduct
offline computations using the rate equations and parameters from RADM2
...
to the \chem{NO_x}-sensitive regime below this threshold. Further, $L_N/Q>2/3$ represent \chem{NO_x}-titration,
i.e. more ozone is expected to be destroyed as a result of increasing \chem{NO_x}, as demonstrated by Equation~\ref{eqn:k01NO}.
Thus, if the majority of air mass lies above the $2/3$ threshold, \chem{NO_x}-titration is expected to occur as
ozone loss caused by \chem{NO_x}-titration compensated ozone production by the air mass of which $L_N/Q<2/3$.
	...
	\begin{wrapfigure}{o}{0.5\textwidth}
		...
		\caption[Ozone convective tendency]{{\label{fig:2006/ltngsens_o3conv}
...
which means that the atmosphere is \chem{NO_x}-sensitive. With $0.1\times$
...
are within the \chem{NO_x}-titration regime, reversal in the ozone production trend is also expected as computed
from the model.
...
\begin{enumerate}
\item Lightning-generated \chem{NO_x} is partially but primarily responsible for the ozone enhancement observed by
...
\chem{NO_x} experiences super-linear increases.
\item Excessive \chem{NO_x} in the upper troposphere causes \chem{NO_x}-titration, which causes ozone loss in both
...
corresponding to the maxima of \chem{CO} over the northeast corner of Texas and along Blue Ridge Mountain as shown in
...
As shown in Figure~\ref{fig:2006/anthsens_vert}{\bf c}, both \chem{NO_x} mixing ratios and \chem{NO_x} chemistry
...
Biogenic emission contributes significantly to tropospheric ozone production, emitting over $7\times$ the VOC from
...
ozone chemical tendency over August. \chem{CO} has a similar change in VMR ($+11\pm3\,\unit{ppbv}$) from the
addition of biogenic emissions, but the enhancement is not due to local chemical production ($\Delta_{chem}=-2\pm5\,\unit{ppbv}$).
Rather, most of the enhancement in \chem{CO} in the upper troposphere is due to convective transport of enhanced \chem{CO}
...
\chem{CO} and $\sim5\,\unit{ppbv}$ \chem{O_3} enhancement due to biogenic emission.
...
Under the base case condition, the simulated \chem{NO_x} mixing ratio is seen rapidly increasing during the first 5
days of the simulation by almost a factor of 10. Validation against SCIAMACHY \chem{NO_2} retrieval shows that
the model is simulating too much high \chem{NO_x} events while properly predicting the mode of the distribution.
...
Other than the enhanced ozone mixing ratio, chemical and convective tendencies within the anticyclone region also
...
($0.1\times$) the emission factor. Using the three simulations, it is concluded that the decreases in \chem{HO_x} radicals as a result of
increasing \chem{NO_x} cause the response of \chem{NO_x} to be superlinear to {\lnox}. By evaluating the
radical termination pathways, it is also concluded that \chem{NO_x}-titration is indeed occurring at the level of
maximum \chem{NO_x} enhancement and thus reducing the potential ozone enhancement with increasing
...
VOC-controlled changes, biogenic emission leads to major changes via perturbation of upper tropospheric \chem{NO_x}. Since the
...
\chem{NO_x} in the base case scenario, our result does not necessarily contradict with \citet{Li:2005ss}, who
showed that anthropogenic (\chem{NO_x}) emission is the dominant source of the ozone enhancement under a {\lnox}
