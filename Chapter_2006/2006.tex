...
During the 28-day period shown in Figure~\ref{fig:2006/precip_ts}(a), WRF predicted 71\,\unit{mm} area mean
...
events are under-predicted (Fig.~\ref{fig:2006/precipts}{\bf b}).  Considering the model error in the mean area prediction for individual day, 12 out of
...
\subsubsection{Validation against TES}
	...
	Normalized frequency distributions (\unit{\%/ppbv}) of TES and WRF-Chem ozone mixing ratios between {\bf (a)}
...
The above validation against TES shows that while ozone is overestimated, the existence of the ozone enhancement and its location are well-simulated.
...
\citep{Tarasick:2007dq} and satellite retrievals \citep{Nassar:2008mw}.
	...
	\begin{wrapfigure}{o}{0.5\textwidth}
		...
		ozonesonde launches at Huntsville, Alabama and the relative bias against the measured ozone mixing ratios. Horizontal dashed lines indicate
...
the primary contributions being smoothing errors and dependency on the a priori. Except for instances of poor spatiotemporal coincidences, correlation
...
observations are highly susceptible to local variabilities and outliers.
...
TES and MOPITT show that \chem{CO} is predicted to be more well-mixed horizontally and vertically than observed. Constrained by anthropogenic emission sources and coastal boundaries,
PBL \chem{CO} VMRs enjoy higher correlations between model and remote sensing data than in the upper troposphere. However, this correlation rapidly drops off for higher altitudes.
...
the left with a negative tail. Negative values in tropospheric column are permitted because of the retrieval process, which
