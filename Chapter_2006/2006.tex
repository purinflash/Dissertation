...
\textbf{(a)} NLDN CG daily flash count ($\times10^4$) within the analysis domain, with WRF-simulated
...
for a rough estimate of the CG fraction in the area according to \citet{Boccippio:2001ys}. However, despite precipitation being
underestimated, CG flash count is approximately 10 times higher than observed by NLDN, which has close to 100\% DE within
the analysis region, which is inconsistent with the result from Chapter~\ref{ch:lightning}. This inconsistency is likely a consequence
of precipitation diagnostics behaving differently than when it was used in the previous chapter, which may be related to one of
the bugs recently discovered within the convective transport module described in Appendix~\ref{a-sec:bug/ctrans}.
...
the distribution truncates prematurely at $\sim200$\,\unit{flashes/grid/day} and compensates with overestimation
below 100 (Fig.~\ref{fig:2006/ltngdist}b). Due to the nonlinear response of ozone chemistry to {\lnox}, such adjustment is
...
is expected. This decision is made because the suggested alternatives have no more or no less merits than the default setting.
