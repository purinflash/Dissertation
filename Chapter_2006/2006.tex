...
the Troposphere.} satellite retrievals \citep{Luo:2007ly,Ho:2009lh} and aircraft measurements \citep{Luo:2007vn,Lopez:2008ys}.
	...
	\begin{sidewaysfigure}[htbp]
		...
		\end{center}
	    	\caption[TES/WRF-Chem \chem{CO} comparisons]{\textbf{TES/WRF \chem{CO} comparisons} --- First row for each panel is the WRF-Chem
		\chem{CO} profiles in ppbv mapped onto TES pressure coordinates after applying Equation \ref{eqn:TES-AK}. Second row is the
		TES profile. Horizontal dashed line indicates the 300\,\unit{hPa} level. Thrid row shows the TES transect and the  9730\,\unit{m}
		geopotential contour at 300\,\unit{hPa} from WRF. Time indicated is the 30$^\circ$N-crossing time.} \label{fig:2006/cotes}
	\end{sidewaysfigure}
...
through its averaging kernel \citep{Shim:2007kx}. In the following validation exercise, we are simply going to compare WRF-Chem's output to the TES \chem{CO}
product using the procedure outlined in Equation~\ref{eqn:TES-AK} to mirror the comparison done with ozone.  Analysis of \chem{O_3}-\chem{CO} correlation
will be revisited in Section~\ref{sec:2006/discussion}.
