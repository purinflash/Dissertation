...
\subsection{Overview of lightning physics}\label{ssec:intro/lightning/overview}
	...
	Discharging roughly $4\times10^8\,\unit{J\,flash^{-1}}$, lightning channels may reach a temperature in excess of $10,000\,\unit{K}$ during the initial return stroke. This rapidly heated channel then expands and generates an outward-propagating shockwave, which is observed as thunder from afar. Finally, both the channel itself and the shock front is then rapidly cooled through mixing. This rapid heating and cooling has been attributed to as the source for {\lnox} (see \ssect{intro/lightning/lnox}).
